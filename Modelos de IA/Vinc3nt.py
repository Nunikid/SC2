...
class Vinc3nt(sc2.BotAI):
	...
	async def build_refinery(self):
		for th in self.townhalls.ready:
			...
			for vg in vgs:
				if await self.can_place(UnitTypeId.REFINERY, vg.position) and self.can_afford(UnitTypeId.REFINERY) and self.structures(UnitTypeId.BARRACKS).ready.amount + self.already_pending(UnitTypeId.BARRACKS) > 0 and self.already_pending(UnitTypeId.REFINERY) < 1:
					workers = self.workers.gathering
					if workers:
	...
	async def morebarracks(self):
		...
		if(
			...
			and self.structures(UnitTypeId.BARRACKS).ready.amount + self.already_pending(UnitTypeId.BARRACKS) < 8
			and self.can_afford(UnitTypeId.BARRACKS)
			):
				workers = self.workers.gathering
				if(
